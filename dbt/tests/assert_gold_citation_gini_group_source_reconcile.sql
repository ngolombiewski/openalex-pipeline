-- Independent source oracle for every published pooled-group/cohort/age cell.
-- This deliberately does not call the shared model macro.
with eligible_papers as (
    ...
    select
        ...
        publication_year,
        case
            when primary_topic_subfield_id = '{{ var("subfield_ai") }}'
                then 'ai'
            when primary_topic_subfield_id = '{{ var("subfield_cv_pr") }}'
                then 'cv_pr'
            else 'rest_cs'
        end as cited_group,
        counts_by_year
