...
from extraction.types import ResumeTarget, RunSummary, YearOutcome
...
def run(settings: Settings) -> RunSummary:
    ...
    2. If plan.target is None: log "all complete", return summary with
       stopped_reason="all_complete" and empty year list.
    3. Otherwise, iterate years ascending starting from plan.target.year.
       For each year:
       ...
       c. On DriftDetected: discard the year via storage.discard_year(),
          ...
          on success.
    4. After all years processed: return summary with
...
def _process_year(
    ...
    - Resume (resume_target is not None): the first request uses
      ...
      raises DriftDetected before any disk mutation.
    ...
    4. Call storage.write_page() with the records and next_cursor.
    5. If next_cursor is None: break (last page reached).
